...
async function login(isPreview = false) {
  ...
  try {
    await api('/foxcave/api/admin/login', { method: 'POST', body: JSON.stringify({ preview: isPreview, password: loginForm.elements.password.value }) });
  ...
  if (!['cancelled', 'declined'].includes(request.status)) {
    ...
    options.filter(([status]) => status !== request.status).forEach(([status, label]) => {
      ...
      button.addEventListener('click', async () => {
        ...
        try { await api(`/foxcave/api/admin/requests/${request.id}`, { method: 'PATCH', body: JSON.stringify({ status }) });
...
function slotCard(slot) {
  ...
  button.addEventListener('click', async () => {
    button.disabled = true;
    try { await api(`/foxcave/api/admin/slots/${slot.id}`, { method: 'PATCH', body: JSON.stringify({ isOpen: !slot.isOpen }) }); await load(); }
    catch (error) { message.textContent = error.message; button.disabled = false; }
  });
...
async function load() {
  try {
    const [requests, result] = await Promise.all([api('/foxcave/api/admin/requests'), api(`/foxcave/api/slots?month=${month.value}`)]);
...
document.querySelector('#slot-form').addEventListener('submit', async (event) => {
  ...
  try {
    await api('/foxcave/api/admin/slots', { method: 'POST', body: JSON.stringify({
...
document.querySelector('#admin-logout').addEventListener('click', async () => {
  try { await api('/foxcave/api/admin/logout', { method: 'POST', body: '{}' }); workspace.hidden = true; loginForm.hidden = false; }
  catch (error) { message.textContent = error.message; }
});
load();
